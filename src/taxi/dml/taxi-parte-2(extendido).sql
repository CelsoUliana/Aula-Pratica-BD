...
FROM taxi.motorista m
INNER JOIN taxi.fila f
ON m.cnh = f.cnh;
...
SELECT z.zona
    FROM taxi.zona z
    WHERE z.zona IN (SELECT DISTINCT f.zona FROM taxi.fila f);

-- Zonas que não receberam algum taxi na fila (com aninhamento / IN)
SELECT z.zona
    FROM taxi.zona z
    WHERE z.zona NOT IN (SELECT DISTINCT f.zona FROM taxi.fila f);